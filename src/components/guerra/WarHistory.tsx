...
const getWarTags = (war: WarHistoryEntry, avgDest: number) => {
  const tags = [];
  if (avgDest === 100) tags.push({ label: "War Perfeita", color: "bg-primary/20 text-primary border-primary/50", icon: Flame });
  else if (avgDest >= 90) tags.push({ label: "Destruição Alta", color: "bg-primary/20 text-primary border-primary/50", icon: FireIcon });
  
  // Lógica simples para "Disputada" (assumindo que score é string "30-29")
  if (war.score) {
    const [us, them] = war.score.split('-').map(Number);
    if (Math.abs(us - them) <= 2) tags.push({ label: "Disputada", color: "bg-primary/20 text-primary border-primary/50", icon: Swords });
  }
        ...
        <CardHeader className="border-b border-border py-4 space-y-4">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <h3 className="font-bold text-white flex items-center gap-2 text-lg">
                <Swords className="w-5 h-5 text-primary"/> Histórico de Batalhas
            </h3>
            ...
            <div className="flex gap-2 w-full md:w-auto">
                <div className="relative flex-1 md:w-48">
                    ...
                    <Input 
                        ...
                        className="pl-9 bg-background border-border text-white focus-visible:ring-primary h-9"
                    />
                </div>
              <Select 
                  value={filter} 
              ...
              >
                  <SelectTrigger className="w-[110px] bg-background border-border text-white h-9">
...
function HistoryItem({ war }: { war: WarHistoryEntry }) {
    ...
    const getResultStyle = (result: string | null) => {
        switch (result) {
            case "win": return { border: "border-l-success", text: "text-success", bg: "hover:bg-success/5", badge: "bg-success/20 text-success" };
            case "loss": return { border: "border-l-destructive", text: "text-destructive", bg: "hover:bg-destructive/5", badge: "bg-destructive/20 text-destructive" };
            default: return { border: "border-l-muted-foreground", text: "text-muted-foreground", bg: "hover:bg-accent/5", badge: "bg-muted/20 text-muted-foreground" };
        }
    };
    ...
    return (
        <AccordionItem value={war.id} className="border-b border-border">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between w-full gap-4 pr-4">
                    ...
                    <div className="flex items-center gap-4 text-left">
                        <div className="flex flex-col">
                            <div className="flex items-center gap-2">
                                ...
                                {war.isLeague && <Badge variant="secondary" className="text-[10px] bg-primary/20 text-primary border-primary/30">CWL</Badge>}
                            </div>
                    ...
                    <div className="flex items-center gap-6 sm:gap-8 w-full sm:w-auto justify-between sm:justify-end">
                        ...
                        <div className="text-right min-w-[60px]">
                            <span className="text-[10px] text-muted-foreground font-bold uppercase block">Placar</span>
            ...
            <AccordionContent className="bg-background px-4 py-4 border-t border-border/50">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                            <div>
                                ...
                                <Progress value={avgDest} className="h-2 bg-accent" indicatorClassName={avgDest > 90 ? "bg-success" : "bg-primary"} />
                            </div>
                            
                            <div className="flex justify-between items-center p-3 bg-card rounded-lg border border-border">
                                <span className="text-sm text-muted-foreground">Total de Ataques</span>
                                <span className="font-bold text-white">{war.attacks.length} <span className="text-muted-foreground text-xs">/ {war.size * (war.isLeague ? 1 : 2)}</span></span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-2">
                         <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-2">
                            <Medal className="w-4 h-4 text-primary"/> MVP da Batalha
                        </h4>
                        {mvp ? (
                            <div className="flex items-center gap-3 p-3 bg-card rounded-lg border border-primary/20 relative overflow-hidden group">
                                <div className="absolute inset-0 bg-gradient-to-r from-primary/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"/>
                                <div className="h-10 w-10 rounded-full bg-muted border border-border flex items-center justify-center font-bold text-muted-foreground">
                            ...
                            <p className="text-xs text-muted-foreground italic">Dados insuficientes para calcular MVP.</p>
                        )}
                        
                        <div className="mt-4 pt-2 border-t border-border">
                             <p className="text-[10px] text-muted-foreground text-center">
                                ID da Guerra: <span className="font-mono text-muted-foreground">{war.id.substring(0,8)}...</span>
                             </p>
                        </div>
                    </div>
                </div>
